/*
 *	(C)opyright 2008-2009 Hanns Holger Rutz. All rights reserved.
 */
 
/**
 *	@version	0.12, 15-Apr-09
 *	@author	Hanns Holger Rutz
 */
NuagesSuhl {
	classvar <>verbose		= false;	// to create a verbose server
	classvar <>debug 		= false;
	classvar <>print 		= false;
//	classvar <>simulate	= false; // true;
//	classvar <>pulseCutOff	= 4000;
	
	classvar <>ludgerGain			= 0.4;
	classvar <>markusGain			= 1.0;
	classvar <>johannesGain			= 1.0;
	classvar <>ludgerOutChannel		= 4;
	classvar <>markusOutChannel		= 5;
	classvar <>johannesOutChannel	= 6;
	classvar <>auxOutChannel		= 7;
	
	classvar <default;

	var <pompe, <bus;
//	var <synthAna, <groupAna;
//	var <patternTime, <patternHisto;
//	var <simuTape;
	var <synthSum;
	var <synthAux;
	
//	var numBufs, bufNumFrames, numBands, spects, pulseCutOffIdx, pulseVals;
	
//	var lastHistoUpdate = 0, <maxHisto;
	...
	*new {
	...
	prInit {
		...
		if( verbose, {
			Server.default		= Server( \nuages, VerboseNetAddr.wrap( Server.default.addr ),
									Server.default.options.copy );
			Server.default.makeWindow;
		});

		default					= this;
		pompe					= Wolkenpumpe.new;
		o						= Server.default.options;
		o.device					= "Mobile I/O 2882 [2600]"; // "Fireface 800 (393)";
		o.numOutputBusChannels		= 18;
		o.numInputBusChannels		= 18;
		o.numAudioBusChannels		= 512; // 256;
		Wolkenpumpe.masterIndex		= 0;
		Wolkenpumpe.masterChannels	= 4;
		Wolkenpumpe.soloIndex		= 8; // 6; // 8;
//		Wolkenpumpe.maxProcChannels	= 4;

		pompe.uf.addAudioGenerator( NuagesIrbeneUnit );
		pompe.uf.addAudioGenerator( NuagesLudgerUnit );
		pompe.uf.addAudioGenerator( NuagesMarkusUnit );
		pompe.uf.addAudioGenerator( NuagesJohannesUnit );
		
//		pompe.uf.addAudioFilter( NuagesRTPulseModUnit );
//		pompe.uf.addAudioFilter( NuagesRTBufGrainUnit );
//		pompe.uf.addAudioFilter( NuagesRTHilbertUnit );
		
		UpdateListener.newFor( pompe, { arg upd; upd.remove; fork { this.prInitAfterBoot }}, \booted );
	}
	...
	*run {
	...
	prInitAfterBoot {
		var s, sr, maxNumChannels, bndl, masterOutU, masterOutP;
//		var buf, dlyBufGetN, cutOffIdx, respAna, cosTable, imag;
		
		s	= pompe.server;
		bus	= Bus( \audio, s.options.numOutputBusChannels + NuagesIrbeneUnit.channel, 1 );
		

// BUG: MeterManager.java doesn't seem to send nuages-meter4, so we do it here explictly
 	maxNumChannels = 4;
	(1..maxNumChannels).do({ arg numCh;
		SynthDef( "nuages-meter" ++ numCh, { arg i_kOtBs, t_trig;
...
//		SynthDef( \irbeneAux, { arg mainBus, auxBus, fade = 0;
//			var fade2, mainSig;
//			
//			fade2	= 1 - fade;
//			mainSig	= In.ar( mainBus, Wolkenpumpe.masterChannels );
//			ReplaceOut.ar( mainBus, mainSig * fade2.sqrt );
//			Out.ar( auxBus, Mix( mainSig ) * Wolkenpumpe.masterChannels.reciprocal * fade.sqrt );
//		}, [ nil, nil, 0.1 ]).send( s );
		
//		SynthDef( \irbeneAna, { arg bus, cutOff = 8000, cutOffD = 0, clipThresh = 0.005, postBoost = 32, bufNum, numBufs;
//			var inp, flt, clip, trig, count, countTrig, countTrigD, phase, bufDurH, step, bufSig, trigDelay, index,
//			    subFrames;
//			    
//			inp			= In.ar( bus );
//			flt			= HPF.ar( HPF.ar( inp, cutOff - cutOffD ), cutOff + cutOffD );
//			clip			= (flt.abs - clipThresh).max( 0.0 ) * flt.sign;
//			bufDurH		= BufDur.kr( bufNum ) / (2 * numBufs );
//			trigDelay		= bufDurH;
//			trig			= TDelay.ar( clip, trigDelay );
//			countTrig		= Impulse.ar( 1 );
//			countTrigD	= Delay1.ar( countTrig );
//			count		= PulseCount.ar( trig, countTrigD );
//		//	trigD		= DelayN.ar( trig, bufDurH, bufDurH );
//		//	trigD		= Delay1.ar( trig );
//		//	phase		= Silent.ar;
//			subFrames		= BufFrames.kr( bufNum ) / numBufs;
//			phase		= Phasor.ar( 0, 1, 0, subFrames );
//		if( debug, { phase.poll( 10, "phase" )});
//			step  = Stepper.ar( trig, 0, 0, numBufs - 1 );
//		if( debug, { step.poll( 10, "step" )});
//			bufSig		= flt * postBoost;
//		//	bufSig		= clip * postBoost;
//		//bufSig = SinOsc.ar( 441 );
//			index		= phase + (step * subFrames);
//			BufWr.ar( bufSig, bufNum, index, 0 );
//		if( debug, { bufSig.poll( 10, "bufSig" )});
//			
//		//	ReplaceOut.ar( bus, trig );
//		//	count.poll( countTrig, "Pulses" );
//			SendTrig.ar( trig, step, phase );
//		}).send( s );
		
//		SynthDef( \irbeneSum, { arg bus, ludgerGain = 1.0, markusGain = 1.0, johannesGain = 1.0;
//			var inp;
//			inp = Mix( In.ar( bus, Wolkenpumpe.masterChannels )) * Wolkenpumpe.masterChannels.reciprocal;
//			Out.ar( ludgerOutChannel, inp * ludgerGain );
//			Out.ar( markusOutChannel, inp * markusGain );
//			Out.ar( johannesOutChannel, inp * johannesGain );
//		}).send( s );

//		numBufs		= 8;
//		bufNumFrames	= 32;
//		buf			= Buffer.alloc( s, numBufs * bufNumFrames, 1 );
//		groupAna		= Group( s );

		s.sync;
//		synthAna		= Synth( \irbeneAna, [ \bufNum, buf.bufnum, \numBufs, numBufs, \postBoost, 50, \bus, bus.index ], groupAna );
//		if( simulate, { this.startSimulator });
		
//		synthSum		= Synth( \irbeneSum, [ \bus, Wolkenpumpe.masterIndex,
//					                       \ludgerGain, ludgerGain, \markusGain, markusGain, \johannesGain, johannesGain ],
//					        pompe.masterGroup, \addAfter );

//		synthAux		= Synth( \irbeneAux, [ \mainBus, Wolkenpumpe.masterIndex, \auxBus, auxOutChannel ],
//					        pompe.masterGroup, \addAfter );

		sr			= s.sampleRate;
//		dlyBufGetN	= bufNumFrames.div( 2 ) / sr;
//		cutOffIdx		= (pulseCutOff / sr * bufNumFrames).asInteger;
//		numBands		= bufNumFrames.div( 2 ) - cutOffIdx;
//		spects		= 0.0 ! numBands ! numBufs;
//		pulseVals		= 0.0 ! (bufNumFrames * numBufs);
//		cosTable		= Signal.fftCosTable( bufNumFrames );
//		imag			= Signal.newClear( bufNumFrames );
//		pulseCutOffIdx = (pulseCutOff / sr * bufNumFrames).asInteger;
//		
//		this.prResetHisto;
//
//		respAna = OSCpathResponder( s.addr, [ '/tr', synthAna.nodeID ], { arg time, resp, msg;
//			var step, phase, off, spect, pattern;
//		//	step  = (msg[ 2 ].asInteger - 1).wrap( 0, numBufs - 1 );
//			step  = msg[ 2 ].asInteger;
//			phase = msg[ 3 ].asInteger;
//			off   = step * bufNumFrames;
////			("Request : " ++ step).postln;
//			{
//				buf.getn( off, bufNumFrames, { arg array;
////					"HUHU".postln;
//					array = array.rotate( phase.neg );
//					array.do({ arg x, i; pulseVals[ off + i ] = x.abs });
//					spects[ step ] = array.fft( imag, cosTable ).magnitude
//						.copyRange( pulseCutOffIdx, pulseCutOffIdx + numBands - 1 );
////					clpseGUIUpdate.instantaneous( pulseVals, step, phase );
//					spects[ step ] = spects[ step ].normalize;
//					spect = spects[ step ];
//					pattern = 0;
//					(spect.differentiate.copyToEnd( 1 ) >= 0).do({ arg rise;
//						pattern = pattern << 1;
//						if( rise, { pattern = pattern | 1 });
//					});
//					if( patternTime[ pattern ].notNil, {
//						if( print, {[ "Trigger", pattern.asStringToBase( 2, 13 ),
//						              (time - patternTime[ pattern ]).round( 0.001)].postln });
//					});
//					patternHisto[ pattern ] = patternHisto[ pattern ] + 1;
//					this.changed( \pulse, pattern );   // do this _before_ updating patternTime
...
//		}).add;
//		
//		UpdateListener.newFor( synthAna, { arg upd, synth;
//			"synthAna died".warn;
//			upd.remove;
//			respAna.remove;
//		}, \n_end );
...
//Routine.run({
//	4.wait;
masterOutU	= NuagesDummyOutputUnit( s );
masterOutU.setPreferredNumOutChannels( Wolkenpumpe.masterChannels );
masterOutU.setAudioInputBus( pompe.masterBus );
bndl			= OSCBundle.new;
masterOutP	= NuagesProc.newToBundle( bndl, s );
masterOutP.setNeverPausingToBundle( bndl, true );
masterOutP.setUnitToBundle( bndl, masterOutU );
masterOutP.setAudioOutputBusToBundle( bndl, pompe.masterBus );
masterOutU.playToBundle( bndl );
bndl.send( s );
pompe.tryChanged( \procAdded, masterOutP, nil );
~masterOutP = masterOutP;
//});
		this.prMakeGUI;
	}
	
	prMakeGUI {
		var spectColors, ggSlid, ggUser, noiseFloor, localMinThresh, localMaxThresh, bounds, clpseGUIUpdate, updPulse,
		    ggThreshSlid, win, specClipThresh, xscale, yscale, ggAuxSlid;
		
		bounds	= pompe.gui.panel.bounds;
		win		= pompe.gui.win;
//		ggSlid	= JSCMultiSliderView( win, Rect( bounds.right + 2, bounds.bottom - 200, 116, 76 ))
//			.elasticMode_( 1 )
//			.resize_( 5 )
//			.drawRects_( false )
//			.drawLines_( true )
...
//		//	.showIndex_( true )
//			.value_( 0.0.dup( bufNumFrames * numBufs ));
//		
//		pulseCutOff = 4000;
//		noiseFloor = -40;
//		localMinThresh = -30;
//		localMaxThresh = -10;
//		spectColors = [  // from niklas werner's sonasound!
...
//
//		ggUser = JSCUserView( win, Rect( bounds.right + 2, bounds.bottom - 122, 116, 120 ))
//			.resize_( 5 )
//			.canFocus_( false )
//			.relativeOrigin_( true );
//
//		xscale = ggUser.bounds.width / numBufs;
//		yscale = ggUser.bounds.height / numBands;
//				
//		ggUser.drawFunc_({ arg v;
//				var rect, db;
////				var b = v.bounds, xscale, yscale, rect, db;
////				xscale = b.width / numBufs;
////				yscale = b.height / numBands;
//				JPen.width = 2;
//				spects.do({ arg spect, x;
//			//		if( x == 0, { spect.postln });
//					spect.do({ arg val, y;
//			//			JPen.fillColor = spectColors[ (val * 256).asInteger.clip( 0, 255 )];
//						db = val.ampdb;
//						JPen.fillColor = spectColors[ db.linlin( noiseFloor, 0, 0, spectColors.size - 1 ).asInteger ];
//						rect = Rect( x * xscale, (numBands - (y + 1)) * yscale, xscale, yscale );
//						JPen.fillRect( rect );
////						if( (y > 0) && (y < (spect.size - 1)), {
////							if( db < localMinThresh, {
////								if( (val < spect[ y - 1 ]) and: { val < spect[ y + 1 ]}, { // local min
////									JPen.strokeColor = Color.red;
////									JPen.strokeRect( rect.insetAll( 1.5, 1.5, 1.5, 1.5 ));
////								});
////							}, { if( db > localMaxThresh, {
////								if( (val > spect[ y - 1 ]) and: { val > spect[ y + 1 ]}, { // local max
...
//			});
//			
//		specClipThresh = ControlSpec( 0.001, 0.02, \exp );
//			
//		ggThreshSlid = JSCMultiSliderView( win, Rect( bounds.right + 2, bounds.bottom - 226, 116, 22 ))
//			.elasticMode_( 1 )
...
////			.readOnly_( true )
//			.canFocus_( false )
//			.value_([ specClipThresh.unmap( 0.005 )])
//			.action_({ arg view;
//				synthAna.set( \clipThresh, specClipThresh.map( view.value.first ));
//			});

//		ggAuxSlid = JSCMultiSliderView( win, Rect( bounds.right + 50, 4, 15, 100 ))
//			.elasticMode_( 1 )
//			.resize_( 3 )
//			.isFilled_( true )
//			.valueThumbSize_( 0 )
...
//			.value_([ 0.0 ])
//			.action_({ arg view;
//				synthAux.set( \fade, view.value.first );
//			});

//		clpseGUIUpdate = Collapse({ arg array, step, phase;
////			"DOING".postln;
//			ggSlid.value = pulseVals;
//			ggUser.refresh;
//		}, 0.1 );
//		
//		updPulse = UpdateListener.newFor( this, { arg upd, obj;
//			clpseGUIUpdate.instantaneous;
//		}, \pulse );
//		ggUser.onClose = { updPulse.remove; clpseGUIUpdate.cancel };
	}
		
//	prResetHisto {
//		patternTime	= Array.fill( 1 << (numBands - 1), nil );
//		patternHisto	= Array.fill( 1 << (numBands - 1), 0 );
//	}
}