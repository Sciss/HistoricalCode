 ...
 *	Changelog:
 *
 *	@version	0.10, 18-Apr-09
 *	@author	Hanns Holger Rutz
 */
NuagesUCOsc : NuagesUGenControlSynth {
	var wave		= 0;
	var freq		= 1.0;
	var curve		= 1.0;
	var low		= 0.0;
	...
	*new { arg server;
	...
	protMakeDef { arg defName, numInChannels, numChannels;
		^SynthDef( defName, {
			...
			var sine, tri, saw, square, sig;
			
			sine		= SinOsc.kr( freq );
			tri		= LFTri.kr( freq );
			saw		= LFSaw.kr( freq );
			square	= LFPulse.kr( freq, mul: 2, add: -1 ); // so it goes from -1 to 1 like the others
			
//			sig		= MulAdd( SelectX.kr( wave, [ sine, tri, saw, square ]), 0.5, 0.5 );
			sig		= (SelectX.kr( wave, [ sine, tri, saw, square ]) * 0.5 + 0.5).pow( curve );
