...
extends JScrollBar
implements AdjustmentListener, TimelineView.Listener,
		   DynamicListening /*, PreferenceChangeListener */
{
    ...
    private static final Color	colrSelection   	= new Color( 0x00, 0x00, 0xFF, 0x2F ); // GraphicsUtil.colrSelection;
	 ...
	 */
    public TimelineScroll( TimelineView doc )
    {
        super( HORIZONTAL );
        this.view    = doc;
		...
		timelineSpan	= view.getTimeline().getSpan();
		timelineVis		= view.getSpan();
		for( timelineLenShift = 0; (timelineSpan.getLength() >> timelineLenShift) > 0x3FFFFFFF; timelineLenShift++ );
		recalcTransforms();
		recalcBoundedRange();
...
//        HelpGlassPane.setHelp( this, "TimelineScroll" );
    ...
    {
		final int len	= (int) (timelineSpan.getLength() >> timelineLenShift);
    ...
    private void recalcTransforms()
    {
		...
		if( !timelineSpan.isEmpty() ) {
			scale           = (double) (recentSize.width - trackMargin) / (double) timelineSpan.getLength();
			if( timelineSel != null ) {
...
//			(timelineVis.getStop() < timelineLen) &&
			...
			stop	= Math.min( timelineSpan.stop, Math.max( timelineSpan.start, start ) + timelineVis.getLength() );
...
//				if( (stop > start) && ((start != timelineVis.getStart()) || (stop != timelineVis.getStop())) ) {
			if( stop > start ) {
				...
				final PerformableEdit edit = TimelineView.Edit.scroll( this, view, timelineVis ).perform();
...
// ---------------- PreferenceChangeListener interface ---------------- 
	...
	public void setCatch( boolean onOff )
	{
		...
		if( !(timelineVis.contains( timelinePos ))) {
			...
			final long	stop	= Math.min( timelineSpan.stop, start + timelineVis.getLength() );
			start				= Math.max( timelineSpan.start, stop - timelineVis.getLength() );
			if( stop > start ) {
				timelineVis = new Span( start, stop );
				final PerformableEdit edit = TimelineView.Edit.scroll( this, view, timelineVis ).perform();
...
// ---------------- TimelineListener interface ---------------- 
    ...
    {
		timelineSpan	= e.getView().getTimeline().getSpan();
		for( timelineLenShift = 0; (timelineSpan.getLength() >> timelineLenShift) > 0x3FFFFFFF; timelineLenShift++ );
		recalcTransforms();
		recalcBoundedRange();
        repaint();
...
// we're listening to ourselves
    ...
    public void adjustmentValueChanged( AdjustmentEvent e )
    {
		...
		final Span		oldVisi		= view.getSpan();
