...
object Obj {
  def read[T <: Txn[T]](in: DataInput, tx: T)(implicit acc: tx.Acc): Obj[T] = Impl.read(in, tx)
  ...
  implicit def serializer[T <: Txn[T]]: TxSerializer[T, Obj[T]] = Impl.serializer
  ...
  type AttrMap    [T <: Txn[T]]             = evt.Map.Modifiable[T, String, Obj]
