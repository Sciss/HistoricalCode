...
import de.sciss.lucre.stm.TxSerializer
...
abstract class CollectionSerializer[T <: Exec[T], A, That <: Traversable[A]] extends TxSerializer[T, That] {
  def newBuilder: mutable.Builder[A, That]
  def empty     : That

  def peer      : TxSerializer[T, A]
...
final class ListSerializer[T <: Exec[T], A](val peer: TxSerializer[T, A])
