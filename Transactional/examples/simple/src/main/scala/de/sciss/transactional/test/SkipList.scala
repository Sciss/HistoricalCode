...
//   def newBuilder[ A : Ordering : MaxKey ]: MBuilder[ A, CC[ A ]] = new MSetBuilder( empty[ A ])
   ...
   trait KeyObserver[ /* @specialized( Int, Long ) */ -A ] {
       ...
       */
      @txn def keyUp( key : A ) : Unit
      /**
       * Notifies the observer that a given key
       * is demoted to a lower (more dense) level
       */
      @txn def keyDown( key : A ) : Unit
   }

   object NoKeyObserver extends KeyObserver[ Any ] {
      @txn def keyUp( key : Any ) {}
      @txn def keyDown( key : Any ) {}
   }
}
...
// TTT
@txn trait SkipList[ /* @specialized( Int, Long ) */ A ]
...
//   override def empty: SkipList[ A ] = SkipList.empty[ A ]( ordering, MaxKey( maxKey ))
    ...
    */
   @txn def isomorphicQuery( compare: A => Int ) : A
    ...
    *
    * @param   v  the key to remove
    * @return  `true` if the key was in the list, `false` otherwise
    */
...
//   implicit final def maxKeyHolder : MaxKey[ A ] = MaxKey( maxKey )
    ...
    */
   final def maxGap : Int = (minGap << 1) + 1
}